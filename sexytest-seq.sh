...
# verify slow test rounds.  Or Specify the chunk sizes in $SEQ.
#
# -r/-w skips the write/read test.  The default is to perform both.
# $VALGRIND is supported as described in sexytest-valgrind.sh.
#
...
# Do the tests.
source sexytest-valgrind.sh;
[ $# -gt 0 -a "x$1" = "x-w" ] \
	|| ./sexywrap -x $valgrind ./sexytest-seq -r "$url" "$disk" $seq;
[ $# -gt 0 -a "x$1" = "x-r" ] \
	|| ./sexywrap -x $valgrind ./sexytest-seq -w "$url" "$disk" $seq;
